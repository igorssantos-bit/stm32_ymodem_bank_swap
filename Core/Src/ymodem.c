...
static HAL_StatusTypeDef ReceivePacket(uint8_t *p_data, uint32_t *p_length, uint32_t timeout) {
	...
	uint8_t char1;
	*p_length = 0;
	status = HAL_UART_Receive(&hcom_uart[COM1], &char1, 1, timeout);
	if (status == HAL_OK) {
		switch (char1) {
		case SOH: {
		...
		break;
		case CA:{
			if ((HAL_UART_Receive(&hcom_uart[COM1], &char1, 1, timeout) == HAL_OK) && (char1 == CA)) {
				packet_size = 2;
			} else {
				status = HAL_ERROR;
			}
		...
		*p_data = char1;
		if (packet_size >= PACKET_SIZE ) {
			status = HAL_UART_Receive(&hcom_uart[COM1], &p_data[PACKET_NUMBER_INDEX], packet_size + PACKET_OVERHEAD_SIZE, timeout);
				...
				default:
					...
					if (aPacketData[PACKET_NUMBER_INDEX] != (0xFFU & packets_received)) {
						...
						if (packets_received == 0) {
							...
							if (FLASH_Write(flashdestination, (uint32_t*) ramsource, packet_length/4) == FLASHIF_OK) {
...
COM_StatusTypeDef Ymodem_Transmit (uint8_t *p_buf, const uint8_t *p_file_name, uint32_t file_size) {
	...
	while (( !ack_recpt ) && ( result == COM_OK )) {
		/* Send Packet */
		HAL_UART_Transmit(&hcom_uart[COM1], &aPacketData[PACKET_START_INDEX], PACKET_SIZE + PACKET_HEADER_SIZE, NAK_TIMEOUT);
...
#endif /* CRC16_F */
		/* Wait for Ack and 'C' */
		if (HAL_UART_Receive(&hcom_uart[COM1], &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK) {
			if (a_rx_ctrl[0] == ACK) {
				ack_recpt = 1;
			} else if (a_rx_ctrl[0] == CA) {
				if ((HAL_UART_Receive(&hcom_uart[COM1], &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK) && (a_rx_ctrl[0] == CA)) {
					HAL_Delay( 2 );
	...
	while ((size) && (result == COM_OK )) {
		...
		while (( !ack_recpt ) && ( result == COM_OK )) {
			...
			HAL_UART_Transmit(&hcom_uart[COM1], &aPacketData[PACKET_START_INDEX], pkt_size + PACKET_HEADER_SIZE, NAK_TIMEOUT);
			/* Send CRC or Check Sum based on CRC16_F */
#ifdef CRC16_F    
...
#endif /* CRC16_F */
			/* Wait for Ack */
			if ((HAL_UART_Receive(&hcom_uart[COM1], &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK) && (a_rx_ctrl[0] == ACK)) {
				ack_recpt = 1;
				if (size > pkt_size) {
					p_buf_int += pkt_size;
					size -= pkt_size;
