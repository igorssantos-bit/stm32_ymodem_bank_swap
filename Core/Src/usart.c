...
void MX_USART1_UART_Init(void)
{
  ...
  huart_p1.p_hdma = (DMA_TypeDef *) GPDMA1;
  huart_p1.rx_channel = LL_DMA_CHANNEL_1;
...
bool uart_read_byte(huart_port_t* huart, void *byte){
	...
	size_t dmaPos = huart->rxBufSize -
			LL_DMA_GetBlkDataLength(huart->p_hdma, huart->rx_channel);
...
bool uart_read_loop(huart_port_t* huart, void *p_dest, size_t len, uint32_t timeout){
	...
	while(!status && (HAL_GetTick() - t0) < timeout){
		status = uart_read_string(huart, p_dest, len);
	}
	return status;
}
